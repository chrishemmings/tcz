...
dbref query_find_object(dbref player,const char *name,int prefered,unsigned char need_control,unsigned char private)
{
      dbref thing;

      setreturn(ERROR,COMMAND_FAIL);
      if(Blank(name)) return(player);
      thing = match_prefered(player,player,name,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,prefered,MATCH_OPTION_DEFAULT);
...
void query_address(CONTEXT)
{
     ...
     thing = query_find_object(player,params,SEARCH_PREFERED,0,0);
     if(Valid(thing)) {
        str_data.length = 0;
...
void query_area(CONTEXT)
{
     ...
     thing = query_find_object(player,arg1,SEARCH_PREFERED,0,0);
     if(!Valid(thing)) return;
     area = query_find_object(player,arg2,SEARCH_PREFERED,0,0);
...
void query_areaname(CONTEXT)
{
     ...
     dbref thing = query_find_object(player,params,SEARCH_PREFERED,0,0);
...
{
     dbref thing = query_find_object(player,arg1,SEARCH_PREFERED,0,1);
     if(!Valid(thing)) return;
...
void query_credit(CONTEXT)
{
     ...
     if(val1 > 0) {
        ...
        object = query_find_object(player,arg1,SEARCH_PREFERED,0,0);
...
/* ---->  {J.P.Boggis 24/09/2000}  Return prefered date/time format  <---- */
...
{
     dbref thing = query_find_object(player,params,SEARCH_PREFERED,0,1);
...
{
     dbref thing = query_find_object(player,params,SEARCH_PREFERED,1,1);
...
void query_exists(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     thing = match_prefered(player,player,params,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT & ~MATCH_OPTION_NOTIFY);
     if(Valid(thing)) {
...
{
     dbref object = query_find_object(player,arg1,SEARCH_PREFERED,((Level4(db[player].owner) || Experienced(db[player].owner)) ? 0:1)|0x2,0);
     char  *ptr; 
...
{
     dbref thing = query_find_object(player,params,SEARCH_PREFERED,((Level4(db[player].owner) || Experienced(db[player].owner)) ? 0:1)|0x2,0);
     if(!Valid(thing)) return;
...
{
     dbref thing = query_find_object(player,params,SEARCH_PREFERED,(Level4(Owner(player)) || Experienced(Owner(player))) ? 0:1,0);
...
{
     dbref         thing = query_find_object(player,arg1,SEARCH_PREFERED,2,0);
     unsigned char article_setting = 0,upper;
...
void query_next(CONTEXT)
{
     ...
     thing = original = query_find_object(player,arg1,SEARCH_PREFERED,0,0);
     if(!Valid(thing)) return;
...
void query_object(CONTEXT)
{
     ...
     thing = query_find_object(player,arg1,SEARCH_PREFERED,1,1);
     if(!Valid(thing)) return;
...
void query_parent(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     if(!Blank(params)) thing = query_find_object(player,params,SEARCH_PREFERED,1,1);
...
{
     dbref object = query_find_object(player,arg1,SEARCH_PREFERED,0,0);
