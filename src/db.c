...
#define _FRIEND_FLAGS_H_ARRAYS
#include "friend_flags.h"
#undef  _FRIEND_FLAGS_H_ARRAYS

#define _FIELDS_H_ARRAYS
#include "fields.h"
#undef  _FIELDS_H_ARRAYS

#define _SIGNALLIST_H_ARRAYS
#include "signallist.h"
#undef  _SIGNALLIST_H_ARRAYS

#define _OBJECTLIST_H_ARRAYS
#include "objectlists.h"
#undef  _OBJECTLIST_H_ARRAYS

#include "flagset.h"
#include "search.h"

#define _MATCH_H_ARRAYS
#include "match.h"
#undef  _MATCH_H_ARRAYS

#include "html.h"

#define _QUOTA_H_ARRAYS
#include "quota.h"
#undef  _QUOTA_H_ARRAYS
...
void db_create(void)
{
	...
	writelog(SERVER_LOG,0,"RESTART","Unable to create required room #%d (New Character Start.)",START_LOCATION);
     ...
     /* ---->  (#4)  Global Commands  <---- */
     sprintf(description,"Place %%c%%lglobal compound commands%%x in this room.  They will be accessable from anywhere on %s.",tcz_short_name);
...
dbref db_read_object(FILE *f,dbref i,int version)
{
     ...
     if(version > 84) {
        ...
        if(objectversion > DATABASE_REVISION) {
           writelog(SERVER_LOG,1,"READ","Cannot read object #%d because it is revision %d (Revisions upto %d are supported.)",i,objectversion,DATABASE_REVISION);
           return(NOTHING);
