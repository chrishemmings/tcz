...
unsigned char html_connect_character(struct descriptor_data *d,const char *data,unsigned char store)
{
       ...
       if(guest || !(!(name = html_lookup("NAME",1,1,1,1,1)) || !*name)) {
          if(!error) {
                   ...
                   if(!guest)
                      switch(ok_character_name(NOTHING,NOTHING,buffer)) {
                             case 0:
                                  break;
                             case 2:
	                          error = html_error(d,0,"Sorry, the maximum length of your prefered character name is 20 characters.",title,back,backurl,HTML_CODE_ERROR);
                                  break;
                             case 3:
	                          error = html_error(d,0,"Sorry, your prefered character name must be at least 4 characters in length.",title,back,backurl,HTML_CODE_ERROR);
                                  break;
                             case 6:
             ...
             if(!error && create) {
                      ...
                      switch(ok_password(password)) {
                             case 0:
                                  if(!strcasecmp(password,name))
                                     error = html_error(d,0,"Sorry, your prefered password can't be the same as your name.",title,back,backurl,HTML_CODE_ERROR);
                                  break;
                             case 1:
                             case 2:
                             case 4:
                                  ...
                                  break;
                             case 3:
                                  error = html_error(d,0,"Sorry, your prefered password must be at least <I>6 characters</I> in length.",title,back,backurl,HTML_CODE_ERROR);
                                  break;
                             default:
		   ...
		   } else error = html_error(d,0,"Please specify your prefered password twice (I.e:  Type it into the <I>PASSWORD</I> and <I>VERIFY PASSWORD</I> boxes.)",title,back,backurl,HTML_CODE_ERROR);
		}
                ...
                if(!error) {
                   ...
                   if(!error) {
                      ...
                      setfield(user,EMAIL,settextfield(email,2,'\0',getfield(user,EMAIL),buffer),0);
	    ...
	    if((!(post = 0) && !strncasecmp(ptr,"GET ",4)) || ((post = 1) && !strncasecmp(ptr,"POST ",5))) {
		  ...
		  if(!(!Blank(resource) && !strcasecmp(resource,"INDEX")) && (html_init(getstart,NOTHING) || html_init(poststart,postlen) || !Blank(resource))) {
			...
			if(!strcasecmp("IMAGE",data) || !strcasecmp("IMAGES",data)) {
			   ...
			   output(d,NOTHING,1,0,0,"<B>Your prefered name:</B> &nbsp; <INPUT NAME=NAME TYPE=TEXT SIZE=20 MAXLENGTH=20 VALUE=\"%s\"><P><B>Password:</B> &nbsp; <INPUT NAME=PASSWORD TYPE=PASSWORD TYPE=TEXT SIZE=20 MAXLENGTH=32 VALUE=\"%s\"> &nbsp; &nbsp; &nbsp; &nbsp; <B>Verify password:</B> &nbsp; <INPUT NAME=VERIFY TYPE=PASSWORD TYPE=TEXT SIZE=20 MAXLENGTH=32 VALUE=\"%s\"><BR><I>(Please type your prefered password in <B>BOTH</B> of the above boxes (For verification.))</I><P>",(params && (data = html_lookup("NAME",1,1,1,1,1))) ? data:"",(params && (data = html_lookup("PASSWORD",1,1,1,1,1))) ? data:"",(params && (data = html_lookup("VERIFY",1,1,1,1,1))) ? data:"");
			   ...
			   if(!Blank(email)) {
				 ...
				 if(!Blank(name)) {
				    if(strlen(name) <= 20) {
					  ...
					  sprintf(buf,"Sorry, we already have a request with your E-mail address (<A>%s</A>) in the queue of requests for new characters.  This request was made on <I>%s</I>.<P>Please ensure that the E-mail address you gave is correct and valid, otherwise we will not be able to contact you.<P>Please click on the <B>BACK</B> button of your browser to return to the previous page and re-enter your prefered character name and your E-mail address.<P>If you experience any difficulties, please send E-mail to <A HREF=\"mailto:%s\">%s</A>.",email,date_to_string(rtime,UNSET_DATE,NOTHING,FULLDATEFMT),tcz_admin_email,tcz_admin_email);
					  html_error(d,1,buf,"NEW CHARACTER REQUEST","Back to the character creation form...",html_server_url(d,0,1,"createform"),HTML_CODE_ERROR|HTML_CODE_HEADER);
				       }
				    } else {
				       writelog(HTML_LOG,1,"REQUEST","Request Denied:  New character request received from descriptor %d (%s)  -  Prefered character name is too long.",d->descriptor,d->hostname);
				       sprintf(buffer,"Sorry, your prefered character name is too long  -  It must be 20 characters or less in length.<P>Please click on the <B>BACK</B> button of your browser to return to the previous page and re-enter your prefered character name and your E-mail address.<P>If you experience any difficulties, please send E-mail to <A HREF=\"mailto:%s\">%s</A>.",tcz_admin_email,tcz_admin_email);
				    ...
				    writelog(HTML_LOG,1,"REQUEST","Request Denied:  New character request received from descriptor %d (%s)  -  Prefered character name not specified.",d->descriptor,d->hostname);
				    sprintf(buffer,"Sorry, you must enter your prefered character name to make a request for a new character.<P>Please click on the <B>BACK</B> button of your browser to return to the previous page and re-enter your prefered character name and your E-mail address.<P>If you experience any difficulties, please send E-mail to <A HREF=\"mailto:%s\">%s</A>.",tcz_admin_email,tcz_admin_email);
				    html_error(d,1,buffer,"NEW CHARACTER REQUEST","Back to the character creation form...",html_server_url(d,0,1,"createform"),HTML_CODE_ERROR|HTML_CODE_HEADER);
				 }
			      } else {
