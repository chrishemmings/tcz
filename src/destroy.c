...
unsigned char destroy_object(dbref player,dbref object,unsigned char char_ok,unsigned char log,unsigned char queue,unsigned char nested)
{
	 ...
	 if(Readonly(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" is Read-Only  -  You can't %s %s.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroy":"purge",(Typeof(object) == TYPE_CHARACTER) ? "them":"it");
	 ...
	 if(Permanent(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" is permanent  -  You can't %s %s.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroy":"purge",(Typeof(object) == TYPE_CHARACTER) ? "them":"it");
	    command_type = cached_commandtype;
	    return(0);
	 }
	 ...
	 if(RoomZero(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"Room Zero"ANSI_LRED") can't be %s  -  It's required internally.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(Root(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"The Supreme Being"ANSI_LRED") can't be %s  -  They're required internally.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(Start(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"New Character Starting Room"ANSI_LRED") can't be %s  -  It's required internally.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(Global(object)) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"Command Last Resort"ANSI_LRED") can't be %s  -  It's required internally.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(object == maint_owner) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"Maintenance Owner"ANSI_LRED") can't be %s.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),getcname(player,object,1,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(object == bbsroom) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"%s BBS"ANSI_LRED") can't be %s.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),tcz_full_name,(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 } else if(object == bankroom) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" ("ANSI_LYELLOW"The Bank of %s"ANSI_LRED") can't be %s.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),unparse_object(player,object,0),tcz_short_name,(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 }
	 ...
	 if(Typeof(object) == TYPE_CHARACTER) {
	    if(!char_ok && !Level4(player)) {
	       ...
	       output(p,player,0,1,0,ANSI_LRED"Sorry, only Apprentice Wizards/Druids and above may %s characters.",(queue) ? "destroy":"purge");
	       command_type = cached_commandtype;
	       return(0);
	    } else if(!char_ok && in_command) {

	       /* ---->  Character being destroyed in compound command?  <---- */
	       output(p,player,0,1,0,ANSI_LRED"Sorry, a character can't be %s from within a compound command.",(queue) ? "destroyed":"purged");
	       command_type = cached_commandtype;
	       return(0);
	    } else if(Level4(object) || Experienced(object)) {

	       /* ---->  Experienced Builder or Apprentice Wizard/Druid or above being destroyed?  <---- */
	       output(p,player,0,1,0,ANSI_LRED"Sorry, an Experienced Builder or an Apprentice Wizard/Druid or above can't be %s.",(queue) ? "destroyed":"purged");
	       command_type = cached_commandtype;
	       return(0);
	    } else if(!char_ok && Being(object)) {

	       /* ---->  'Being' character being destroyed?  <---- */
	       output(p,player,0,1,0,ANSI_LRED"Sorry, a Being can't be %s.",(queue) ? "destroyed":"purged");
	       command_type = cached_commandtype;
	       return(0);
	    } else if(!char_ok && !Ashcan(object)) {

	       /* ---->  Non-ASHCAN'ed character being destroyed?  <---- */
	       output(p,player,0,1,0,ANSI_LRED"Sorry, only characters with their "ANSI_LYELLOW"ASHCAN"ANSI_LRED" flag set can be %s.",(queue) ? "destroyed":"purged");
	       command_type = cached_commandtype;
	       return(0);
	    }
	 }
	 ...
	 if(!((Typeof(object) == TYPE_VARIABLE) || (Typeof(object) == TYPE_PROPERTY) || (Typeof(object) == TYPE_ARRAY))) {
	    ...
	    if(ok_to_destroy && (Typeof(object) == TYPE_CHARACTER) && Connected(object)) {
	       output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" can't be %s because they are still connected.",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),getcname(player,object,1,0),(queue) ? "destroyed":"purged");
	       command_type = cached_commandtype;
	       return(0);
	    }
	 }
	 ...
	 if(!ok_to_destroy) {
	    output(p,player,0,1,0,ANSI_LRED"Sorry, %s"ANSI_LWHITE"%s"ANSI_LRED" can't be %s for the above reason(s).",Article(object,LOWER,(nested) ? INDEFINITE:DEFINITE),getcname(player,object,1,0),(queue) ? "destroyed":"purged");
	    command_type = cached_commandtype;
	    return(0);
	 }
	 ...
	 if(log) {
	    if(db[player].owner != db[object].owner) {
		  ...
		  writelog(DESTROY_LOG,1,(queue) ? "DESTROY":"PURGE","%s(#%d) %s %s (%s, owned by %s(#%d)) within compound command %s(#%d).",getname(player),player,(queue) ? "destroyed":"purged",unparse_object(ROOT,object,0),names[Typeof(object)],getname(db[object].owner),db[object].owner,getname(current_command),current_command);
		     else writelog(DESTROY_LOG,1,(queue) ? "DESTROY":"PURGE","%s(#%d) %s %s (%s, owned by %s(#%d).)",getname(player),player,(queue) ? "destroyed":"purged",unparse_object(ROOT,object,0),names[Typeof(object)],getname(db[object].owner),db[object].owner);
	    }
	 }

	 if(!in_command) output(p,player,0,1,0,ANSI_LRED"%s "ANSI_LWHITE"%s"ANSI_LRED" %s.",names[Typeof(object)],unparse_object(player,object,0),(queue) ? "destroyed":"purged");
...
void destroy_destroy(CONTEXT)
{
     ...
     if(!Blank(arg1)) {
        object = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
        ...
        if(object != player) {
           if(object != db[player].owner) {
                    ...
                    if(!Readonly(object)) {
                       if(elementfrom != INVALID) {
                          ...
                          if(!in_command) {
                             ...
                             output(getdsc(player),player,0,1,0,"%s"ANSI_LWHITE"%s"ANSI_LGREEN" %s.",scratch_buffer,unparse_object(player,object,0),(queue) ? "destroyed":"purged");
			  }
		    ...
		    } else output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, that dynamic array is Read-Only  -  You can't %s any of its elements.",(queue) ? "destroy":"purge");
		 } else if(destroy_object(player,object,0,1,queue,0)) {
                    if(log) {
                       verify = punctuate(reason,2,'.');
                       writelog(ADMIN_LOG,1,(queue) ? "DESTROY":"PURGE","%s(#%d) %s character %s(#%d)  -  REASON:  %s",getname(player),player,(queue) ? "destroyed":"purged",scratch_buffer,object,verify);
		    }
                    setreturn(OK,COMMAND_SUCC);
		 }
	      } else if((Typeof(object) == TYPE_ARRAY) && (elementfrom != NOTHING)) {
                 if(Level3(db[player].owner)) output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only %s elements of a dynamic array you own or one that's owned by someone of a lower level than yourself.",(queue) ? "destroy":"purge");
                 else output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only %s elements of a dynamic array you own.",(queue) ? "destroy":"purge");
	      } else if(Level3(db[player].owner)) output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only %s something you own or something owned by someone of a lower level than yourself.",(queue) ? "destroy":"purge");
                 else output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only %s something you own.",(queue) ? "destroy":"purge");
	   } else output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can't %s the owner of the compound command currently being executed.",(queue) ? "destroy":"purge");
	} else output(getdsc(player),player,0,1,0,ANSI_LGREEN"If you no-longer need your character, please ask a Wizard/Druid or above to %s them for you ("ANSI_LWHITE"PLEASE NOTE"ANSI_LGREEN":  Once your character has been %s, there will be no way of retrieving them, or their possessions.)",(queue) ? "destroy":"purge",(queue) ? "destroyed":"purged");
     } else output(getdsc(player),player,0,1,0,ANSI_LGREEN"Please specify what you'd like to %s.",(queue) ? "destroy":"purge");
}
