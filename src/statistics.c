...
void stats_contents(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     object = match_prefered(player,player,(Blank(arg1)) ? "here":arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
...
void stats_rank(CONTEXT)
{
        ...
        *tmp = '\0';
        if(!Blank(scratch_return_string)) {
         if(string_prefix("totaltimeconnected",scratch_return_string)) order = RANK_TOTAL;
           ...
           /* ---->  Constraints  <---- */
           if(((order == RANK_QUOTA) || (order == RANK_QUOTALIMIT) || (order == RANK_QUOTAEXCESS)) && !Level4(Owner(player))) {
              output(p,player,0,1,0,ANSI_LGREEN"Sorry, only Apprentice Wizards/Druids and above may rank characters by Building Quota in use or their Building Quota limit.");
              return;
	   } else if((order == RANK_CSIZE) || (order == RANK_SIZE)) {
              if(in_command) {
     ...
     if(!Blank(arg2)) {
        if(!types) {
           ...
           rwho = match_prefered(player,player,arg2,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
