...
dbref parse_link_command(dbref player,dbref object,const char *name,unsigned char prompt)
{
      ...
      command = match_prefered(player,player,name,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_COMMAND,MATCH_OPTION_DEFAULT);
...
dbref parse_link_destination(dbref player,dbref object,const char *name,int flags)
{
      ...
      if(Blank(name)) return(NOTHING);
      if(!strcasecmp(name,"home")) return(HOME);
      destination = match_prefered(player,player,name,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
