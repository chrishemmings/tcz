            ...
            case TYPE_CHARACTER:
                 ...
                 if(will_fit(object,destination)) {
                    ...
                    if((oldloc != Location(object)) && !Invisible(Location(object))) {
                       ...
                       output_except(Location(object),object,NOTHING,0,1,0,ANSI_LGREEN"%s"ANSI_LWHITE"%s"ANSI_LGREEN" has arrived.",Article(object,UPPER,INDEFINITE),getcname(NOTHING,object,0,0));
...
unsigned char move_enter(dbref player,dbref destination,unsigned char autolook)
{
	 ...
	 if(destination != old) {
	    ...
	    if(Valid(old)) {
		  ...
		  output_except(old,player,NOTHING,0,1,0,ANSI_LGREEN"%s"ANSI_LWHITE"%s"ANSI_LGREEN" has left.",Article(player,UPPER,DEFINITE),getcname(NOTHING,player,0,0));
	       }
	    }
        ...
        if(!Immovable(vehicle)) {
           if(could_satisfy_lock(player,vehicle,0)) {
              ...
              if(strcasecmp(dest,"home")) {
                 ...
                 destination = match_prefered(player,player,dest,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT & ~MATCH_OPTION_NOTIFY);
...
void move_getdrop(CONTEXT)
{
     ...
     thing = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
     if(!Valid(thing)) return;

     /* ---->  Work out where to put it  <---- */
     if(!Blank(arg2)) {
        container = match_prefered(player,player,arg2,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
...
void move_give(CONTEXT)
{
     ...
     if(!Blank(arg1)) {
        if((recipient = lookup_character(player,arg1,1)) != NOTHING) {
           object = match_prefered(player,player,arg2,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
...
void move_remote(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     loc  = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
     if(!Valid(loc)) return;
...
void move_ride(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     if(!Blank(arg1)) {
        vehicle = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_THING,MATCH_OPTION_DEFAULT);
...
void move_teleport(CONTEXT)
{
     ...
     if(!Blank(arg2)) {
        destination = match_prefered(player,player,arg2,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
