...
void server_initialise_telnet(int descriptor)
{
     ...
     write(descriptor,_IAC""_DO""_LINE,3);      /*  Line-by-line mode prefered  */
           ...
           for(d = descriptor_list; d; d = dnext) {
			...
			if(!(!Blank(db[d->player].name) && instring("guest",getname(d->player)))) {
                     ...
                     if(!(d->flags & CONNECTED)) {
                        ...
                        /* ---->  Maximum allowed idle time of connected user exceeded (30 minutes)  <---- */
                        ...
                        server_shutdown_sock(d,0,0);                        
		     } else if(diff > ((15 + (d->warning_level * 5)) * MINUTE)) {

                        /* ---->  Warning messages for every 5 minutes of idle time over 15 minutes  <---- */
                        output(d,d->player,2,0,0,ANSI_WYELLOW"\n[You have been idle for "ANSI_WWHITE"%d minute%s"ANSI_WYELLOW"  -  %d minute%s until automatic disconnection...]\n\n",(15 + (d->warning_level * 5)),Plural(15 + (d->warning_level * 5)),MAX_IDLE_TIME - (15 + (d->warning_level * 5)),Plural(MAX_IDLE_TIME - (15 + (d->warning_level * 5))));
                        d->warning_level++;
		     }
		  }
	   }
    ...
    if(!Blank(buffer) && instring("guest",buffer)) {
       if(!creations || !connections) {
          ...
          if(IsHtml(d)) {
             ...
             sprintf(buffer2,"Back to %s web site...",tcz_full_name);
             html_error(d,1,"Sorry, connection as a Guest character is not allowed  -  Please click on the <B>BACK</B> button of your browser and enter your prefered character name in the '<I>Your prefered name:</I>' box.","CONNECTION REFUSED",buffer2,html_home_url,HTML_CODE_ERROR);
	  } else output(d,d->player,2,0,0,ANSI_LRED"\nSorry, connection as a Guest character is not allowed  -  Please enter '"ANSI_LYELLOW"NEW"ANSI_LRED"' at the '"ANSI_LWHITE"Please enter your name:"ANSI_LRED"' prompt and then enter your prefered character name and your E-mail address to request a new character.\n\n");
          FREENULL(d->name);
          d->clevel = -1;
          return(0);
          ...
          if(IsHtml(d)) {
             ...
             sprintf(buffer2,"Back to %s web site...",tcz_full_name);
             html_error(d,1,"Sorry, Guest characters may no-longer connect from your site due to excessive abuse  -  Please click on the <B>BACK</B> button of your browser and enter your prefered character name in the '<I>Your prefered name:</I>' box.","CONNECTION REFUSED",buffer2,html_home_url,HTML_CODE_ERROR);
	  } else if(d->address != GUEST_LOGIN_ADDRESS) output(d,d->player,2,0,0,ANSI_LRED"\nSorry, Guest characters may no-longer connect from your site due to excessive abuse  -  Please enter your prefered character name at the '"ANSI_LWHITE"Please enter your name:"ANSI_LRED"' prompt and then enter your E-mail address to request a new character.\n\n");
             else output(d,d->player,2,0,0,ANSI_LRED"\nSorry, Guest characters may not connect from %s Guest Telnet login service  -  Please enter your prefered character name at the '"ANSI_LWHITE"Please enter your name:"ANSI_LRED"' prompt and then enter your E-mail address to request a new character.\n\n",tcz_full_name);
          FREENULL(d->name);
          d->clevel = -1;
          return(0);
...
unsigned char server_site_banned(dbref user,const char *name,const char *email,struct descriptor_data *d,int connect,char *buffer)
{
	 ...
	 if(!connect) {
	    if(!creations) {
		  ...
		  sprintf(buf2,"Sorry, character creation is not allowed.  You will need to make a request for a new character to be created for you.<P>To do this, simply enter your prefered character name and your E-mail address in the box below and click on the <B>REQUEST NEW CHARACTER</B> button.  Your request will be placed on a queue and we will be in touch with you shortly.<P><CENTER><FONT SIZE=3><FORM METHOD=GET ACTION=\"%s\"><B>Prefered character name:</B> &nbsp; <INPUT NAME=NAME TYPE=TEXT SIZE=20 MAXLENGTH=20 VALUE=\"%s\"><P><B>E-mail address:</B> &nbsp; <INPUT NAME=EMAIL TYPE=TEXT SIZE=60 MAXLENGTH=128 VALUE=\"%s\"><P><INPUT TYPE=SUBMIT VALUE=\"Request new character...\"><INPUT NAME=DATA TYPE=HIDDEN VALUE=REQUEST><P></FORM></FONT></CENTER>If you experience any difficulties, please send E-mail to <A HREF=\"mailto:%s\">%s</A>.",
	    ...
	    if(d->site) {
		  ...
		  if(IsHtml(d)) {
		     ...
		     sprintf(buf2,"Sorry, new characters can no-longer be created from your site due to excessive abuse.  If you would like a character created, please enter your prefered character name and your E-mail address in the box below and click on the <B>REQUEST NEW CHARACTER</B> button.  Your request will be placed on a queue and we will be in touch with you shortly.<P><CENTER><FONT SIZE=3><FORM METHOD=GET ACTION=\"%s\"><B>Prefered character name:</B> &nbsp; <INPUT NAME=NAME TYPE=TEXT SIZE=20 MAXLENGTH=20 NAME=\"%s\"><P><B>E-mail address:</B> &nbsp; <INPUT NAME=EMAIL TYPE=TEXT SIZE=60 MAXLENGTH=128 VALUE=\"%s\"><P><INPUT TYPE=SUBMIT VALUE=\"Request new character...\"><INPUT NAME=DATA TYPE=HIDDEN VALUE=REQUEST><P></FORM></FONT></CENTER>If you experience any difficulties, please send E-mail to <A HREF=\"mailto:%s\">%s</A>.",html_server_url(d,0,0,NULL),html_encode_basic(name,buf1,&copied,128),html_encode_basic(String(email),buf1 + 256,&copied,256),tcz_admin_email,tcz_admin_email);
		     html_error(d,1,buf2,"CONNECTION REFUSED","Back to the character creation form...",html_server_url(d,0,1,"createform"),HTML_CODE_ERROR);
		  }
	        ...
	        /* ---->  Create new character  <---- */
                switch(ok_character_name(NOTHING,NOTHING,str)) {
                       case 0:
                            ...
                            break;
                       case 2:
                            if(d->clevel == 28) output(d,d->player,2,0,0,ANSI_LRED"\nSorry, the maximum length of your prefered character name is 20 characters.\n\n");
                               else output(d,d->player,2,0,0,ANSI_LRED"\nSorry, the maximum length of your name is 20 characters.\n\n");
                            break;
                       case 3:
                            if(d->clevel == 28) output(d,d->player,2,0,0,ANSI_LRED"\nSorry, your prefered character name must be at least 4 characters in length.\n\n");
			    ...
			    output(d,d->player,2,0,0,ANSI_LRED"\nSorry, a character with the name '"ANSI_LYELLOW"%s"ANSI_LRED"' already exists.  Please choose another name for your new character.\n",str);
                            break;
                       case 5:
                            if(d->clevel == 28) output(d,d->player,2,0,0,ANSI_LRED"\nSorry, your prefered character name can't be blank.\n\n");
