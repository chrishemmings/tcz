...
void request_view(dbref player,const char *reference,unsigned short refno)
{
     ...
     output(p,player,2,1,24,"%sPrefered name:%s"ANSI_LWHITE"%s.%s",IsHtml(p) ? "\016<TR><TH ALIGN=RIGHT WIDTH=25% BGCOLOR="HTML_TABLE_YELLOW"><I>\016"ANSI_LYELLOW:ANSI_LYELLOW"        ",IsHtml(p) ? "\016</I></TH><TD ALIGN=LEFT>\016":"  ",(ptr->name) ? decompress(ptr->name):"<UNKNOWN>",IsHtml(p) ? "\016</TD></TR>\016":"\n");
...
void request_list(dbref player,const char *email,const char *name)
{
     ...
     if(!in_command) {
        if(!IsHtml(p)) {
           output(p,player,0,1,0,"\n Number:  Date:       Administrator:        Prefered name and E-mail address:");
           output(p,player,0,1,0,separator(twidth,0,'-','='));
	} else output(p,player,2,1,0,"\016<TR ALIGN=CENTER BGCOLOR="HTML_TABLE_CYAN"><TH><FONT COLOR="HTML_LCYAN" SIZE=4><I>Number:</I></FONT></TH><TH><FONT COLOR="HTML_LCYAN" SIZE=4><I>Date:</I></FONT></TH><TH><FONT COLOR="HTML_LCYAN" SIZE=4><I>Administrator:</I></FONT></TH><TH><FONT COLOR="HTML_LCYAN" SIZE=4><I>Prefered name and E-mail address:</I></FONT></TH></TR>\016");
     }
     ...
     while(union_grouprange()) {
           ...
           /* ---->  Prefered name and E-mail address  <---- */
