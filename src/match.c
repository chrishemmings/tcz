...
dbref match_object(dbref owner,dbref start,dbref end,const char *searchstr,unsigned char startphase,unsigned char endphase,int types,int options,int subtypes,struct match_data *caller,unsigned char level)
{
      ...
      if(match && (match->phase == MATCH_PHASE_KEYWORD)) {
         ...
         if((*(match->search) == MATCH_TOKEN) && !match->recursed) {
            ...
            if(!(match->options & MATCH_OPTION_PREFERED)) {
               int newtypes = 0;
               int newtype;
...
/*  ENDPHASE   = Ending phase                    (MATCH_PHASE_GLOBAL)  */
/*  TYPES      = Types of objects to match for           (SEARCH_ALL)  */
/*  PREFERED   = Prefered types of object              (SEARCH_THING)  */
/*  OPTIONS    = Matching options              (MATCH_OPTION_DEFAULT)  */
dbref match_prefered(dbref owner,dbref start,const char *searchstr,unsigned char startphase,unsigned char endphase,int types,int prefered,int options)
{
      ...
      if(prefered > 0) {
         ...
         if(types == SEARCH_PREFERED) prefoptions |= MATCH_OPTION_SKIP_INVISIBLE;
         if(!(!Blank(searchstr) && strchr(searchstr,':')))
            result = match_object(owner,start,NOTHING,searchstr,startphase,endphase,types & prefered,prefoptions|MATCH_OPTION_PREFERED,SEARCH_ALL,NULL,0);
               else prefered = 0;
      }

      if(!Valid(result))
         result = match_object(owner,start,NOTHING,searchstr,(!prefered || (startphase > MATCH_PHASE_KEYWORD)) ? startphase:MATCH_PHASE_KEYWORD,endphase,types,options,SEARCH_ALL,NULL,0);
      return(result);
}
	      ...
	      for(*dest-- = '\0'; (dest >= querybuf) && (*dest == ' '); *dest-- = '\0');
              ...
              if(!Blank(querybuf)) {
                 if((preftype = search_name_to_type(querybuf)) == NOTHING)
                    output(getdsc(player),player,0,1,2,ANSI_LGREEN"Sorry, the prefered object type '"ANSI_LWHITE"%s"ANSI_LGREEN"' is unknown.",querybuf);
	               else preftypes |= (type_to_search[preftype]);
	      }
	}
        if(!preftypes) preftypes = SEARCH_PREFERED;
     }

     /* ---->  Perform match  <---- */
     cached_parent = parent_object;
     object = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,preftypes,((val1) ? MATCH_OPTION_DEFAULT:(MATCH_OPTION_DEFAULT & ~MATCH_OPTION_NOTIFY)) | ((val2) ? MATCH_OPTION_PARENT:0));
