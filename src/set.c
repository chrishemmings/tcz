...
#define _FLAGSET_H_ARRAYS
#include "flagset.h"
#undef  _FLAGSET_H_ARRAYS
...
/* ---->  {J.P.Boggis 24/09/2000}  Set prefered date/time format  <---- */
        ...
        if(!Readonly(thing)) {
           if((Typeof(thing) != TYPE_CHARACTER) || !in_command || Wizard(current_command)) {
              if(!Blank(arg2)) {
                 ...
                 /* ---->  Set parent  <---- */
                 parent = match_prefered(player,player,arg2,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
...
void set_unlink(CONTEXT)
{
     ...
     setreturn(ERROR,COMMAND_FAIL);
     thing = match_prefered(player,player,params,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_ROOM|SEARCH_EXIT,MATCH_OPTION_DEFAULT);
     if(!Valid(thing)) return;
