...
#define _SEARCH_H_ARRAYS
#include "search.h"
#undef  _SEARCH_H_ARRAYS
           ...
           if(strcasecmp("email",arg1)) {
              ...
              if(!(all && Blank(arg2))) {
                 if(!Blank(arg1) && !Blank(arg2)) {
                    if(!all) {
                       who = match_prefered(player,player,arg1,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
                       if(!Valid(who)) return;
