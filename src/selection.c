     ...
     *(params - 1) = ' ';
     ...
     if((!val1 || *params) && !selection_do_keyword(params - 1)) {
        ...
        *ptr = '\0';
        ...
        object = match_prefered(player,player,querybuf,MATCH_PHASE_KEYWORD,MATCH_PHASE_GLOBAL,SEARCH_ALL,SEARCH_PREFERED,MATCH_OPTION_DEFAULT);
        if(!Valid(object)) {
           if(in_command && selection_skip_do_keyword(params)) {
              command_textptr = selection_seek_end(command_textptr,&endline,0,0);
              current_line_number = (endline - 1);
     ...
     if(listtype & SEARCH_CONNECTED) {
        switch(listtype & (SEARCH_ARRAY|SEARCH_INDEX|SEARCH_FRIENDS|SEARCH_ENEMIES)) {
               case SEARCH_ARRAY:
               case SEARCH_INDEX:
               ...
               case SEARCH_FRIENDS:
                    ...
                    if(Typeof(object) != TYPE_CHARACTER) {
                       if(!val1) output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only process the %s list of a character  -  %s"ANSI_LWHITE"%s"ANSI_LGREEN" isn't a character.",(listtype & SEARCH_ENEMIES) ? "enemies":"friends",Article(object,UPPER,DEFINITE),unparse_object(player,object,0));
                       if(block) {
                          command_textptr = endptr, *endptr = endchar;
                          current_line_number = endline;
		       }
                       ...
                       if(!val1) output(getdsc(player),player,0,1,0,ANSI_LGREEN"Sorry, you can only process your own %s list.",(listtype & SEARCH_ENEMIES) ? "enemies":"friends");
                       if(block) {
                          command_textptr = endptr, *endptr = endchar;
                          current_line_number = endline;
		       }
                    ...
                    set_conditions_ps(player,(listtype & SEARCH_FRIENDS) ? 1:0,0,0,0,0,SEARCH_CHARACTER,field_flags,(Blank(index)) ? NULL:index,504);
                    ...
                    while(union_grouprange() && ((flow_control & FLOW_MASK_LOOP) == FLOW_NORMAL)) {
                          ...
                          noloops = loop;
                          strcpy(command_item2,String(getfield(grp->cunion->friend.friend,DESC)));
                          strcpy(command_item,getnameid(player,grp->cunion->friend.friend,NULL)); 
